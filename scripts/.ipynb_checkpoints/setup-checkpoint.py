...
def setup(alfvar, onlybasic = False, ncpu=4):
    ...
    if np.nanmax(alfvar.data.ires) > 10. and onlybasic == False:
        ...
        #    for (j, k), _ in np.ndenumerate(getattr(alfvar.sspgrid, iattr)[0]):
        #        print(j, k, end=',')
        ...
        #print('\noriginal velbroad {}s'.format(ndur))
        ...
        ndur = time.time() - tstart
        print('\nparallelized velbroad {}s'.format(ndur))
        ...
        #for (i, j), _ in np.ndenumerate(alfvar.sspgrid.hotspec[0]):
        ...
        #for (k, j, t, z), _ in np.ndenumerate(alfvar.sspgrid.logssp[0]):
        #    print(k, j, t, z, end=',')
        #    alfvar.sspgrid.logssp[:,k,j,t,z] = velbroad(alfvar.sspgrid.lam, alfvar.sspgrid.logssp[:,k,j,t,z],
        #                                                sig0,lamlo,lamhi,smooth, velbroad_simple = 1)
        ...
        if alfvar.imf_type == 3:
            ...
            #for (k, j, t, m, z), _  in np.ndenumerate(alfvar.sspgrid.logsspm[0]):
            #    print(k, j, t, m, z, end=',')
            #    alfvar.sspgrid.logsspm[:,k,j,t,m,z] = velbroad(alfvar.sspgrid.lam, alfvar.sspgrid.logsspm[:,k,j,t,m,z],
            #                                                   sig0,lamlo,lamhi,smooth, velbroad_simple = 1)
