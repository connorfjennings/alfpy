...
def func_2min(inarr):
    """
    only optimize the first 4 parameters for now
    """
...
def alf(filename, alfvar=None, tag='', run='dynesty',
    ...
    global_all_prior = [TopHat(global_prloarr[i], global_prhiarr[i]) for i in range(len(key_list))]
    initial_all_prior = copy.deepcopy(global_all_prior)
    ...
    if run == 'emcee':
        ...
        pickle.dump(res, open('../emcee_chain_{0}_{1}.p'.format(filename, tag), "wb" ) )
        pickle.dump(prob, open('../emcee_prob_{0}_{1}.p'.format(filename, tag), "wb" ) )
