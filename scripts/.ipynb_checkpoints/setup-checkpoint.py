...
def setup(alfvar, onlybasic = False):
    """
    !read in and set up all the arrays
    """
    ...
    # -------------------------------------------------------------------------!
    f20_read_dict_attr = ['lam','solar','nap','nam','cap','cam','fep',
    ...
    print('lam=sspgrid.lam:',lam.shape, lam)
    ...
    tmpzgrid = np.array([-0.5, 0.0, 0.25])
    alfvar.sspgrid.logzgrid2[:alfvar.nzmet3] = np.copy(tmpzgrid[:alfvar.nzmet3])
    ...
    if alfvar.imf_type == 3:
        print("begin: read in 3 parameter IMF models")
        for z in range(alfvar.nzmet3):
            print(" ---- zmet = ",z)
            for m in range(alfvar.nmcut):
                for t in range(alfvar.nage):
                    f22 = np.loadtxt("{0}infiles/VCJ_v8_mcut{1}_{2}_Z{3}"\
    ...
    for i in range(nimf):
        alfvar.sspgrid.imfx1[i] = 0.5 + float(i + nimfoff)/5.0
    ...
    for j in range(alfvar.nzmet):
        f24 = np.loadtxt("{0}infiles/at12_feh{1}_afe+0.0_t08000g4.00"\
                         ".spec.s100".format(ALF_HOME, charz2[j]))
        alfvar.sspgrid.hotspec[:,0,j] = f24[alfvar.nstart-1:alfvar.nend, 1]

        f25 = np.loadtxt("{0}infiles/at12_feh{1}_afe+0.0_t10000g4.00"\
                         ".spec.s100".format(ALF_HOME, charz2[j]))
        alfvar.sspgrid.hotspec[:,1,j] = f25[alfvar.nstart-1:alfvar.nend, 1]

        f26 = np.loadtxt("{0}infiles/at12_feh{1}_afe+0.0_t20000g4.00"\
                         ".spec.s100".format(ALF_HOME, charz2[j]))
        alfvar.sspgrid.hotspec[:,2,j] = f26[alfvar.nstart-1:alfvar.nend, 1]

        f27 = np.loadtxt("{0}infiles/at12_feh{1}_afe+0.0_t30000g4.00"\
                         ".spec.s100".format(ALF_HOME, charz2[j]))
    ...
    print('begin: set up the atm transmission function & sky lines')
    ...
    if np.nanmax(alfvar.data.ires) > 10. and onlybasic == False:
        ...
        for k in range(alfvar.nzmet):
            for j in range(alfvar.nage_rfcn):
                for iattr in temlist:
                    getattr(alfvar.sspgrid, iattr)[:,j,k] = velbroad(alfvar.sspgrid.lam,
                                                            getattr(alfvar.sspgrid, iattr)[:,j,k],
        ...
        for j in range(alfvar.nzmet):
            for i in range(alfvar.nhot):
                alfvar.sspgrid.hotspec[:,i,j] = velbroad(alfvar.sspgrid.lam, alfvar.sspgrid.hotspec[:,i,j],
                                                         sig0,lamlo,lamhi,smooth,
                                                         velbroad_simple = 1)

        alfvar.sspgrid.m7g = velbroad(alfvar.sspgrid.lam,
                                      alfvar.sspgrid.m7g,
                                      sig0, lamlo, lamhi, smooth,
                                      velbroad_simple = 1)
        ...
        for index, xarr in np.ndenumerate(alfvar.sspgrid.logssp[0]):
            ...
            alfvar.sspgrid.logssp[:,k,j,t,z] = velbroad(alfvar.sspgrid.lam,
                                                        alfvar.sspgrid.logssp[:,k,j,t,z],
                                                        sig0,lamlo,lamhi,smooth,
                                                        velbroad_simple = 1)
        #for z in range(alfvar.nzmet):
        #    print('-------- -------- alfvar.nzmet=',z)
        #    for t in range(alfvar.nage):
        #        for j in range(alfvar.nimf):
        #            for k in range(alfvar.nimf):
        #                alfvar.sspgrid.logssp[:,k,j,t,z] = velbroad(alfvar.sspgrid.lam,
        #                                                            alfvar.sspgrid.logssp[:,k,j,t,z],
        #                                                            sig0,lamlo,lamhi,smooth,
        #                                                            velbroad_simple = 1)
        ...
        if alfvar.imf_type == 3:
            for index, xarr in np.ndenumerate(alfvar.sspgrid.logsspm[0]):
                k, j, t, m, z = index
                alfvar.sspgrid.logsspm[:,k,j,t,m,z] = velbroad(alfvar.sspgrid.lam,
        ...
        if alfvar.imf_type == 4:
            for index, xarr in np.ndenumerate(alfvar.sspgrid.sspnp[0]):
                j, t, z = index
                alfvar.sspgrid.sspnp[:,j,t,z] = velbroad(alfvar.sspgrid.lam,
                                                         alfvar.sspgrid.sspnp[:,j,t,z],
                                                         sig0,lamlo,lamhi,smooth,
                                                         velbroad_simple = 1)
            #for z in range(alfvar.nzmet):
            #    for t in range(alfvar.nage):
            #        for j in range(alfvar.nimfnp):
            #            alfvar.sspgrid.sspnp[:,j,t,z] = velbroad(alfvar.sspgrid.lam,
            #                                                     alfvar.sspgrid.sspnp[:,j,t,z],
            #                                                     sig0,lamlo,lamhi,smooth,
            #                                                     velbroad_simple = 1)
